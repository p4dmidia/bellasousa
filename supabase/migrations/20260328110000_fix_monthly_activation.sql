-- Fix Monthly Activation Rule
-- Ensures any purchase by an affiliate activates them for 30 days.
...
-- 2. Update the commission processing function
...
BEGIN
    ...
    IF (NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed')) THEN
        ...
        -- Fetch Config
        SELECT level_commissions, network_depth, leadership_bonus_config, commission_type 
        INTO v_config 
        ...
        -- Identify Buyer
        -- Use both exact match and sanitized phone match to be safe
        ...
        WHERE (email ILIKE NEW.email AND NEW.email IS NOT NULL AND NEW.email != '')
           ...
           OR (regexp_replace(whatsapp, '\D', '', 'g') = regexp_replace(NEW.whatsapp, '\D', '', 'g') 
               AND NEW.whatsapp IS NOT NULL AND NEW.whatsapp != '');

        IF v_buyer_id IS NULL THEN v_buyer_id := NEW.affiliate_id; END IF;

        -- REGRA DE ATIVAÇÃO MENSAL: Qualquer compra ativa o afiliado que comprou
        ...
        -- Process Each Item for Upline commissions
            ...
            FOR j in 0..(v_depth - 1) LOOP
                ...
                IF v_curr_affiliate_id != v_buyer_id THEN
                    IF COALESCE(v_prod_record.is_leadership_item, false) THEN
                        ...
                        END LOOP;
                    ELSE
                        -- MMN Bonus: RESPECT commission_type (Fixed vs Percentage)
                        IF j < JSONB_ARRAY_LENGTH(v_level_commissions) THEN
                            v_comm_val := (v_level_commissions->>j)::NUMERIC;
                            IF v_is_fixed THEN
                                v_amt := v_comm_val * v_qty; -- Fixed R$ per quantity
                            ELSE
                                v_amt := v_item_total * (v_comm_val / 100); -- Percentage
                            END IF;
                        END IF;
                    END IF;
                END IF;
                ...
                IF v_amt > 0 THEN
                    ...
                    VALUES (v_curr_affiliate_id, v_amt, 'commission', 
                           ...
                           ' - Item: ' || (COALESCE(v_item->>'name', 'Produto')) || ' - Pedido #' || SUBSTR(NEW.id::TEXT, 1, 8), 
                           NEW.id);
...
-- 3. Attach the trigger (VITAL STEP)
