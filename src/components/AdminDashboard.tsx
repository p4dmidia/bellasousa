...
export default function AdminDashboard({ onLogout, onNavigateHome }: AdminDashboardProps) {
  ...
  const handleUpdateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      // Se está marcando como concluído e era pendente, distrib        // 2. Multi-Level Commission Distribution (MMN)
        if (newStatus === 'completed' && orderToUpdate.affiliate_id) {
        ...
        // A comissão multinível agora é processada automaticamente pelo banco de dados (trigger)
        // quando o status do pedido é alterado para 'completed'.
        }
